...
![Ticky screenshot](Assets/ticky-screenshot.png)
...
- 업데이트: 우측 하단에 `업데이트 확인됨`이 표시되면 해당 문구를 클릭해 최신 버전으로 업데이트할 수 있습니다. 설치 중에는 다운로드/압축 해제 진행률이 표시됩니다. 업데이트가 없을 때는 현재 버전이 표시됩니다.

## 업데이트 진행률 테스트

실제 GitHub 릴리스나 설치 파일 교체 없이 업데이트 진행률 UI를 확인하려면 아래 스크립트를 실행합니다.

```powershell
.\scripts\Test-UpdateProgress.ps1
```

앱이 실행되면 우측 하단의 업데이트 배너를 클릭해 게이지 동작을 확인할 수 있습니다. 이 테스트는 Debug 실행에서만 활성화되며, 임시 zip 파일을 사용하고 실제 앱 설치 경로를 덮어쓰지 않습니다.

## 후원

커피 한 잔 던져주고, 업데이트를 재촉해주세요.
...
- Update: When `Update available` appears in the bottom-right corner, click it to install the latest version. Download/extract progress is shown while installing. When there is no update, the current version is shown instead.

## Testing Update Progress

Run the script below to test the update progress UI without a real GitHub Release or replacing installed app files.

```powershell
.\scripts\Test-UpdateProgress.ps1
```

When the app opens, click the update banner in the bottom-right corner. This test is only enabled for Debug runs, uses a temporary zip package, and does not overwrite the real install directory.

## Support

Toss it a coffee and nudge the next update along.
